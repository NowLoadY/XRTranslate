...
const DICTIONARY: &[(&str, &str, &str, &str, &str)] = &[
    ...
    (
        ...
        "自动模式会在 CUDA 可用时使用匹配的运行库，否则回退到 CPU。",
        "自動モードはCUDAが利用可能な場合は対応するランタイムを使用し、それ以外はCPUへフォールバックします。",
        "자동 모드는 CUDA를 사용할 수 있으면 일치하는 런타임을 사용하고, 그렇지 않으면 CPU로 대체합니다.",
        "Автоматический режим использует подходящую среду CUDA, если она доступна, иначе переключается на CPU.",
    ),
    (
        "TTS acceleration",
        "TTS 推理加速",
