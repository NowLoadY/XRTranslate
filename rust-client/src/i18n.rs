...
const DICTIONARY: &[(&str, &str, &str, &str, &str)] = &[
    ...
    (
        "Use as translation style",
        "用作翻译风格",
        "翻訳スタイルとして使用",
        "번역 스타일로 사용",
        "Использовать как стиль перевода",
    ),
    (
        "Locate style node",
        "定位风格节点",
    ...
    (
        "Use a text node’s context menu to designate a shared translation style.",
        "右键静态文本节点，选择「用作翻译风格」即可共享并在此编辑。",
        "共有テキストノードのメニューで翻訳スタイルに指定してください。",
        "공유 텍스트 노드의 메뉴에서 번역 스타일로 지정하세요.",
        "Назначьте общий текстовый узел стилем перевода через его контекстное меню.",
    ),
    (
        "Style node is not connected to this model.",
        "风格节点尚未连接到此模型。",
        "スタイルノードがこのモデルに接続されていません。",
        "스타일 노드가 이 모델에 연결되지 않았습니다.",
        "Узел стиля не подключён к этой модели.",
    ),
    (
        "Style text",
        "风格正文",
    ...
    (
        "Remove from saved styles. Current style text is kept.",
        "从已保存的列表移除，保留当前风格正文。",
        "保存済みリストから削除します。現在のスタイル本文は残ります。",
        "저장 목록에서 삭제합니다. 현재 스타일 본문은 유지됩니다.",
        "Удалить из сохранённых стилей. Текущий текст останется.",
    ),
    (
        "Style name",
        "风格名称",
    ...
    (
        "Removed from list. Current text kept.",
        "已从列表移除，当前正文保留。",
        "リストから削除しました。本文は残っています。",
        "목록에서 삭제했습니다. 현재 본문은 유지됩니다.",
        "Удалено из списка. Текущий текст сохранён.",
    ),
    (
        "No saved styles. Current text is still available above.",
        "列表为空，上方仍保留当前风格。",
        "保存済みスタイルはありません。現在の本文は上に残っています。",
        "저장된 스타일이 없습니다. 현재 본문은 위에 남아 있습니다.",
        "Список пуст. Текущий текст остаётся выше.",
    ),
    (
        "Instruction + style",
        "处理指令 + 风格",
    ...
    (
        "Set up translator microphone",
        "设置翻译器麦克风",
        "翻訳マイクを設定",
        "번역기 마이크 설정",
        "Настроить микрофон переводчика",
    ),
    (
        "Follow TTS switch",
        "跟随 TTS 开关",
        "TTS の切り替えに連動",
        "TTS 스위치에 연동",
        "Следовать переключателю TTS",
    ),
    (
        "Microphone output enabled",
        "启用麦克风输出",
        "マイク出力を有効化",
        "마이크 출력 활성화",
        "Включить выход микрофона",
    ),
    (
        "TTS on: translated speech only, including silence while waiting. TTS off: your original microphone.",
        "TTS 开启：只输出合成语音，等待时保持静音。TTS 关闭：输出麦克风原声。",
        "TTS オン：合成音声のみ。待機中は無音。TTS オフ：元のマイク音声。",
        "TTS 켜짐: 합성 음성만 출력하며 대기 중에는 무음. 꺼짐: 원래 마이크 음성.",
        "TTS включён: только синтезированная речь, тишина при ожидании. Выключен: исходный микрофон.",
    ),
    (
        ...
        "自動マイク：合成音声",
        "자동 마이크: 합성 음성",
        "Автомикрофон: синтезированная речь",
    ),
    (
        ...
        "自動マイク：元の音声",
        "자동 마이크: 원래 음성",
        "Автомикрофон: исходный голос",
    ),
    (
        "Select this microphone in VRChat",
        "在 VRChat 中选择此麦克风",
        "VRChat でこのマイクを選択",
        "VRChat에서 이 마이크 선택",
        "Выберите этот микрофон в VRChat",
    ),
    (
        "In VRChat, select the recording end of the chosen virtual cable (for VB-CABLE: CABLE Output).",
        "在 VRChat 中选择所选虚拟声卡的录音端（VB-CABLE 对应 CABLE Output）。",
        "VRChat で仮想ケーブルの録音側を選択します（VB-CABLE は CABLE Output）。",
        "VRChat에서 가상 케이블의 녹음 장치를 선택하세요(VB-CABLE: CABLE Output).",
        "В VRChat выберите записывающий конец виртуального кабеля (VB-CABLE: CABLE Output).",
    ),
    (
        ...
        "仮想オーディオケーブルを入手",
        "가상 오디오 케이블 받기",
        "Получить виртуальный аудиокабель",
    ),
    ...
    ("Music", "音乐", "音楽", "음악", "Музыка"),
    ("Technology", "科技", "テクノロジー", "기술", "Технологии"),
    (
        "Travel and Places",
        "旅行与地点",
        "旅行・場所",
    ...
    ("Research", "研究", "研究", "연구", "Исследования"),
    (
        "Anime and Manga",
        "动漫",
        "アニメ・マンガ",
    ...
    ("Transportation", "交通", "交通", "교통", "Транспорт"),
    ("Workplace", "职场", "職場", "직장", "Рабочее место"),
    (
        "Search domains",
        "搜索领域",
        "分野を検索",
    ...
    (
        "Check title length and characters.",
        "请检查标题的长度和字符。",
        "タイトルの長さ・文字を確認してください。",
        "제목의 길이 및 문자를 확인하세요.",
        "Проверьте длину и символы заголовка.",
    ),
    (
        "Discard",
        "放弃修改",
    ...
    (
        ...
        "打开官方文档并获取 API Key",
        "公式ドキュメントを開いてAPIキーを取得",
        "공식 문서를 열고 API 키 발급받기",
        "Открыть официальную документацию и получить ключ API",
    ),
    (
        "qwen (China Mainland)",
        "qwen (中国大陆)",
    ...
    (
        "Your GPU has less than 3 GiB of VRAM. Local models require at least 3 GiB, so this option is disabled.",
        "你的 GPU 显存不足 3 GiB。本地模型至少需要 3 GiB 显存，因此该选项已禁用。",
        "GPU の VRAM が 3 GiB 未満です。ローカルモデルには 3 GiB 以上が必要なため、この項目は無効です。",
        "GPU VRAM이 3 GiB 미만입니다. 로컬 모델에는 최소 3 GiB가 필요하므로 이 옵션이 비활성화되었습니다.",
        "Объём видеопамяти GPU меньше 3 ГиБ. Для локальных моделей требуется не менее 3 ГиБ, поэтому этот вариант отключён.",
    ),
    ...
    ("Default", "默认", "デフォルト", "기본", "По умолчанию"),
    ("Hand-drawn", "手绘", "手描き", "손그림", "Рисованный"),
    (
        "Download proxy",
        "下载代理",
        "ダウンロードプロキシ",
    ...
    (
        "Pseudo-streaming Default",
        "伪流式默认方案",
        "疑似ストリーミング既定",
        "유사 스트리밍 기본값",
        "Псевдопоток по умолчанию",
    ),
    (
        "Built-in Default",
        "内置默认",
    ...
    ("VIEW /", "视图 /", "表示 /", "보기 /", "ВИД /"),
    ("MODE", "模式", "モード", "모드", "РЕЖИМ"),
    ("SOURCE", "源语言", "入力言語", "원본 언어", "ИСТОЧНИК"),
    (
        "REFERENCE CONTEXT",
        "参考上下文",
        "参照コンテキスト",
    ...
    (
        ...
        "可复制详情，通过 GitHub Issue 或 QQ 群 1009732148 反馈。",
        "詳細をコピーして、GitHub Issue または QQ グループ 1009732148 で報告できます。",
    ...
    (
        ...
        "ヘッダーとフッターを常時表示",
        "머리글과 바닥글 항상 표시",
        "Постоянно показывать заголовок и футер",
    ),
    (
        "None (Disabled)",
        "无（已禁用）",
    ...
    (
        "Display private real-time bilingual subtitles in SteamVR (HMD-locked HUD).",
        "在 SteamVR 中显示私密实时的双语悬浮字幕（HMD 视线锁定 HUD 模式）。",
        "SteamVR内でプライベートなリアルタイムバイリンガル字幕を表示 (HMDロックHUD)。",
        "SteamVR에서 비공개 실시간 이중 언어 자막 표시 (HMD 잠금 HUD).",
        "Отображение личных двуязычных субтитров в SteamVR (HUD с фиксацией на HMD).",
    ),
    (
        "SteamVR In-Game Overlay",
        "SteamVR 游戏内画面叠加",
        "SteamVR ゲーム内オーバーレイ",
        "SteamVR 인게임 오버레이",
    ...
    (
        ...
        "在 VR 游戏中叠加私密双语实时字幕（HMD 视线锁定模式，不受反作弊影响）。",
        "VRゲーム内でプライベートなバイリンガルリアルタイム字幕をオーバーレイ (HMDロック、アンチチート安全)。",
        "VR 게임에서 비공개 이중 언어 실시간 자막 오버레이 (HMD 고정 모드, 안티치트 안전).",
        "Оверлей личных двуязычных субтитров в VR-играх (HUD режим, безопасно для античита).",
    ),
    (
        "VR Spatial HUD Position (HMD-Locked)",
        "VR 空间 HUD 位置（HMD 视线锁定）",
        "VR 空間 HUD 位置 (HMDロック)",
        "VR 공간 HUD 위치 (HMD 고정)",
        "Позиция пространственного HUD в VR (с фиксацией на HMD)",
    ),
    (
        "Display Settings",
        "显示设置",
    ...
    (
        ...
        "最大字幕行数",
        "최대 자막 줄 수",
        "Макс. количество строк субтитров",
    ),
    (
        "Font Size",
        "字体大小",
    ...
    (
        "No active captions (waiting for speech input)...",
        "暂无字幕（等待语音输入）...",
        "アクティブな字幕はありません (音声入力を待機中)...",
        "활성 자막 없음 (음성 입력 대기 중)...",
        "Нет активных субтитров (ожидание речи)...",
    ),
    (
        "Max Lines:",
        "最大行数：",
